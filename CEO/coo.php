...
if (isset($_POST['addcoo'])) {
  ...
  $sql = "INSERT INTO coo (username, employee_code, password, status, postdate) 
          VALUES ('$username', '$employee_code', '$password', '$status', '$postdate')";
    if ($conn->query($sql) === TRUE) {
    echo "COO Added successfully";
    } else {
    echo "Error: " . $sql . "<br>" . $conn->error;
    }
  
  // Redirect to a success page
  header("Location: coo.php");
  // Display a success message using JavaScript
  echo "<script>alert('COO Added successfully!');window.location.href='$_SERVER[PHP_SELF]'</script>";
  exit();
}
?>
                        ...
                        <form method="post">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-footer text-right">
                                    <button type="submit" name="addcoo" class="btn btn-primary">Add</button>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-md">
                                    <tbody>
                                        ...
                                        $status = "active";
                                        if ($status == "active") {
                                            $sql = "UPDATE coo SET status='inactive' WHERE id='$id'";
                                            $result = mysqli_query($conn, $sql);
                                            if ($result) {
                                                echo "<script>location.href='coo.php';</script>";
                                            }
                                        }
                                        }
